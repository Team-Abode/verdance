...
public record DecayPatchConfiguration(Patch patch, List<Distance> distances) implements FeatureConfig {
    ...
    public record Patch(int tries, int horizontalSpread, int verticalSpread) {
        public static final Codec<Patch> CODEC = RecordCodecBuilder.create(instance -> instance.group(
                Codecs.POSITIVE_INT.fieldOf("tries").orElse(128).forGetter(Patch::tries),
                Codecs.NONNEGATIVE_INT.fieldOf("horizontal_spread").orElse(7).forGetter(Patch::horizontalSpread),
                Codecs.NONNEGATIVE_INT.fieldOf("vertical_spread").orElse(3).forGetter(Patch::verticalSpread)
        ).apply(instance, Patch::new));
    }

    public record Distance(int distance, RegistryEntry<PlacedFeature> feature) {
        public static final Codec<Distance> CODEC = RecordCodecBuilder.create(instance -> instance.group(
                Codecs.NONNEGATIVE_INT.fieldOf("distance").forGetter(Distance::distance),
                PlacedFeature.REGISTRY_CODEC.fieldOf("feature").forGetter(Distance::feature)
